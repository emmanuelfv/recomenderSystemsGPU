...
Las diferencias con la versión 1 se encuentran en la aplicacíon del modelo, se
reduce la complejidad de O(M * M) por O(M*K).
*/
...
Eval *DB_to_eval(char *tabla, int registros) {
  ...
  if (registros == 0)
    ...
    sprintf(querty, "SELECT user_id, item_id, rating FROM %s LIMIT %d", tabla,
            registros);
