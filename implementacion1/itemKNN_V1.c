/*
En este documento se presenta el primer sistema de recomendación implementado. ItemKNN

Nos basamos en el algoritmo propuesto en 2004, Item-Based Top-N Recommendation Algorithms
...
for i <- 1 to m
    for j <- 1 to m
        if i != j
            ...
            S <- 0
    for i <- 1 to m
        if S_{i,j} != entre los más grandes k valores en S_{*,i}
            S <- 0
return(S)

Donde:
n = |U| = numero total de usuarios
m = |I| = número total de ítems
R = matriz de recomendaciones de tamaño [n, m]
k = número maximo de items por vector
S = matriz de similitud de items tamaño [m,m]
sim(x,y) = función de evaluación de similitud
...
for i <- 1 to m
    if u_i != 0
        x_i <- 0
for i <- 1 to m
    if xi != entre los N más grandes valores en x
        x_i <- 0
return(x)

Donde:
u = vector de items que ha comprado el usuario activo valores 1, 0
N = numero de items a recomendar
x = vector de recomendaciones para el usuario activo

El algoritmo realizado tiene las siguientes complejidades dado N=|U|, M=|I|:
O(N*M + M^2) para el almacenamiento de los datos.
O(M^2*N) para la construcción del modelo.
O(M^2) para la generación de recomendaciones.

idealmente un sistema de recomendación badado en vecindarios para items tendría
O(M^2), O(M^2*q), O(M*K) para el almacenamiento, construcción y explotación del
modelo respectivamente. 

resultados
construcción del modelo + 1 recomendación
ml100k = 16.37 s
ml1m = 9m29.7 s
